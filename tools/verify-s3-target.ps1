...
Assert-SourcePattern 'if\s*\(\(padDown\s*&\s*KEY_START\)\s*!=\s*0\)\s*\{\s*initializeProjectiles\(\)\s*;\s*initializeEnemy\(\)\s*;' 'P1 START must reset the Enemy and clear all active shells.'
...
Write-Output 'ENEMY_RESET=P1_START_DOWN'
